package ru.itis.dis403.lab04.controller;


import freemarker.template.Template;
import freemarker.template.TemplateException;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import ru.itis.dis403.lab04.service.AirplaneService;

import java.io.IOException;
import java.io.Writer;
import java.util.HashMap;
import java.util.Map;

import freemarker.template.Configuration;


@WebServlet("/airplanes")
public class AirplaneServlet extends HttpServlet {

    private AirplaneService airplaneService = new AirplaneService();

    private Configuration cfg;

    @Override
    public void init() throws ServletException {
        cfg = new Configuration(Configuration.VERSION_2_3_22);
        cfg.setClassForTemplateLoading(AirplaneServlet.class, "/template");
        cfg.setDefaultEncoding("UTF-8");
    }


    @Override
    public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        res.setContentType("text/html;charset=UTF-8");
        airplaneService.fillAttributes(req);


        Map<String, Object> model = new HashMap<>();
        model.put("airplanes", req.getAttribute("airplanes"));
        Template template = cfg.getTemplate("airplanes.ftlh");



        try (Writer out = res.getWriter()) {
            template.process(model, out);
        } catch (TemplateException e) {
            throw new RuntimeException(e);
        }


       // req.getRequestDispatcher("D:\\itis\\javaProject\\ORIS\\lab1_04\\src\\main\\webapp\\WEB-INF\\templates\\airplanes.ftlh").forward(req, res);
    }
}
